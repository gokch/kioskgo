package p2p

import (
	"context"

	"github.com/gokch/kioskgo/file"
	"github.com/ipfs/boxo/routing/http/contentrouter"
	"github.com/ipfs/go-cid"
)

// waitlist 발신 ( 수신 / 송신 )
type Client struct {
	*P2P
	havelist cids // 현재 보유 목록
	waitlist cids // 다운로드 대기 목록
}

func NewClient(ctx context.Context, address string, fs *file.FileStore, clientrouter contentrouter.Client) (*Client, error) {
	if clientrouter == nil {
	}

	p2p, err := NewP2P(ctx, address, fs, clientrouter)
	if err != nil {
		return nil, err
	}

	return &Client{
		P2P: p2p,
	}, nil
}

func (c *Client) Start() {

}

func (c *Client) AddWaitlist() {

}

// 1. waitlist 에서 요청한 cid 를 다운로드 받았을 경우
// 2. 새로운 cid 를 가진 파일을 피어에 올릴 경우
func (c *Client) AddHavelist() {

}

// waitlist 에서 지운다 + havelist 에 추가한다
func (c *Client) recvDownload(ctx context.Context, path string) {
	c.P2P.Download(ctx, cid.Cid{}, path)
}

// peer 에 제공? - TODO: IPNS 를 통해 고유값 지정이 필요!!
func (c *Client) recvUpload(ctx context.Context, path string) {
	c.P2P.Upload(ctx, path)
}
